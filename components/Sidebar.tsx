...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4" style={{ backgroundColor: "#212130" }}>
      ...
      {open && (
        <div className="lg:hidden fixed top-[57px] left-0 right-0 z-40 py-4" style={{ backgroundColor: "#212130" }}>
          {navItems.map(({ id, label }) => (
            <button key={id} onClick={() => scrollTo(id)}
              className="block w-full text-left px-6 py-2.5 text-sm transition-colors hover:text-white"
              style={{ color: active === id ? "#72cc76" : "#9ca3af" }}>
              {label}
            </button>
          ))}
        </div>
      ...
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col fixed top-0 left-0 h-full w-56 z-40" style={{ backgroundColor: "#212130" }}>
        ...
        <nav className="flex-1 py-6 overflow-y-auto">
          {navItems.map(({ id, label }) => (
            <button key={id} onClick={() => scrollTo(id)}
              ...
              style={{ color: active === id ? "#72cc76" : "#6b7280" }}>
              <span className="block h-px flex-1 max-w-[16px] transition-all" style={{ backgroundColor: active === id ? "#72cc76" : "transparent" }} />
